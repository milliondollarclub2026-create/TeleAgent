...
# ---------------------------------------------------------------------------
# Tool 6 — List deals with optional filters
# ---------------------------------------------------------------------------
...
async def list_deals(
    ...
    try:
        ...
        sort_col = "value" if sort_by == "value" else "created_at"
        query = query.order(sort_col, desc=order_desc)

        # Fetch a bit more than cap to detect truncation
        ...
        for r in rows:
            ...
            if updated_str:
                try:
                    ...
                    pass
            if min_days_in_stage is not None and (days_in_stage is None or days_in_stage < min_days_in_stage):
                continue
            deals.append({
