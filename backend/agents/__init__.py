...
class FaridWidgetSpec(BaseModel):
    """One widget spec generated by Farid during onboarding."""
    chart_type: str          # "bar", "pie", "line", "funnel", "kpi"
    title: str
    data_source: str         # "crm_leads", "crm_deals", etc.
    x_field: str
...
class FaridRefinementQuestion(BaseModel):
    """One refinement question generated by Farid for a goal."""
    id: str
    type: str                # "multiselect", "radio", "order"
    question: str
    why: str = ""
    options: list[dict] = Field(default_factory=list)
    options_from_field: Optional[dict] = None  # {table, field}
    default: Any = None


class FaridGoal(BaseModel):
    """One business-specific goal generated by Farid."""
    id: str                  # snake_case, business-specific (e.g. "student_enrollment_funnel")
    name: str
    description: str
    why: str = ""            # data-driven justification
    data_confidence: float = 0.5
    relevant_entities: list[str] = Field(default_factory=list)
    key_fields: dict = Field(default_factory=dict)  # {table: [fields]}
    widgets: list[FaridWidgetSpec] = Field(default_factory=list)
    refinement_questions: list[FaridRefinementQuestion] = Field(default_factory=list)


class FaridBusinessProfile(BaseModel):
    """Business profile inferred by Farid from CRM data."""
    type: str = "unknown"
    summary: str = ""
    primary_entity: str = "deals"
    revenue_entity: str = "deals"


class FaridOnboardingResult(BaseModel):
    """Complete output of Farid's discover_and_plan() — validated before use."""
    business_profile: FaridBusinessProfile = Field(default_factory=FaridBusinessProfile)
    baseline_kpis: list[FaridWidgetSpec] = Field(default_factory=list)
    goals: list[FaridGoal] = Field(default_factory=list)
    entity_labels: dict = Field(default_factory=dict)
